...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bet-group-label.today-label { color: #111; border-color: #111; }
    .bet-group + .bet-group { margin-top: 20px; }
    ...
    .bet-row:hover { background: #fafafa; margin: 0 -8px; padding: 10px 8px; border-radius: 8px; border-bottom: 1px solid transparent; }
    ...
    .bet-date-chip.is-today { background: #111; color: #fff; }
    .bet-date-chip.is-future { background: #f0f0f0; color: #888; }

    .bet-match {
      font-weight: 600; font-size: 0.88rem; color: #111;
      flex: 1; min-width: 160px;
    }
    .bet-match .bet-vs { color: #ccc; font-weight: 400; margin: 0 4px; }
    ...
    .conf-lean   { background: #f9fafb; color: #9ca3af; }

    .bet-edge {
      font-size: 0.9rem; font-weight: 800; white-space: nowrap; min-width: 54px;
      text-align: right;
    }
    ...
    .bet-odds strong { color: #333; font-weight: 700; }

    .bets-footer { margin-top: 14px; font-size: 0.67rem; color: #ccc; }
...
function betPick(e) {
  ...
  if (e.outcome === 'Home Win') return e.home;
  if (e.outcome === 'Away Win') return e.away;
  return e.outcome; // Over 2.5 / Under 2.5 etc
}

function betRow(e, isToday) {
  const pick    = betPick(e);
  const conf    = e.edge >= 15 ? ['Strong', 'conf-strong']
                : e.edge >= 8  ? ['Value',  'conf-value']
                :                ['Lean',   'conf-lean'];
  const edgeClr = e.edge >= 15 ? '#16a34a' : e.edge >= 8 ? '#2563eb' : '#6b7280';
  const chip    = isToday
    ? `<span class="bet-date-chip is-today">Today</span>`
    : `<span class="bet-date-chip is-future">${fmtShort(e.date)}</span>`;
  const odds    = e.best_odds
    ? `<strong>${e.best_odds}</strong> · ${e.best_book}`
    : '';
  return `
    <div class="bet-row">
      ${chip}
      <span class="bet-match">${e.home}<span class="bet-vs">vs</span>${e.away}</span>
      <span class="bet-pick">${pick}</span>
      <span class="bet-confidence ${conf[1]}">${conf[0]}</span>
      <span class="bet-edge" style="color:${edgeClr}">+${e.edge}%</span>
...
function renderEdges() {
  ...
  if (todayBets.length) {
    inner += `
    <div class="bet-group today-group">
      <div class="bet-group-label today-label">Today</div>
      ${todayBets.map(e => betRow(e, true)).join('')}
    </div>`;
  }

  if (upcomingBets.length) {
    inner += `
    <div class="bet-group ${todayBets.length ? 'bet-group' : ''}">
      <div class="bet-group-label">Upcoming · sorted by edge</div>
      ${upcomingBets.map(e => betRow(e, false)).join('')}
    </div>`;
  }

  inner += `<p class="bets-footer">Model prob vs vig-free consensus · bet responsibly</p>`;
  sec.innerHTML = inner;
}
